"""
Script de création d'exécutable (MODE DOSSIER / ONE-DIR) pour Pokémon Analyzer
Génère un dossier complet avec l'exécutable et ses dépendances.
Avantage : Démarrage plus rapide et meilleures performances.
"""
import os
import sys
import subprocess
...
def create_spec_file():
    """Crée le fichier .spec optimisé pour PyInstaller (MODE DOSSIER)"""
    print_step("3/7", "CRÉATION DU FICHIER SPEC (MODE DOSSIER)")

    spec_content = '''# -*- mode: python ; coding: utf-8 -*-
"""
Fichier .spec pour PyInstaller - Pokémon Analyzer (MODE DIR)
Compile l'application en mode dossier pour de meilleures performances
"""
...
# MODE DOSSIER : On ne met QUE les scripts dans l'exe
exe = EXE(
    pyz,
    a.scripts,
    [], # PAS DE BINAIRES/DATA ICI
...
# MODE DOSSIER : On collecte tout dans un dossier
coll = COLLECT(
    exe,
    a.binaries,
    a.zipfiles,
...
'''
    ...
    with open('PokemonAnalyzer_dir.spec', 'w', encoding='utf-8') as f:
        f.write(spec_content)

    print("[OK] Fichier PokemonAnalyzer_dir.spec créé")
    return True
...
def build_with_pyinstaller():
    ...
    print_step("5/7", "CONSTRUCTION DE L'EXÉCUTABLE (MODE DOSSIER)")

    print("[INFO] Lancement de PyInstaller...")
    print("[INFO] Cela est rapide (one-dir)...\n")

    try:
        # Essaye d'abord avec la commande pyinstaller
        # Construit la commande de base
        cmd = ['pyinstaller', 'PokemonAnalyzer_dir.spec', '--clean', '--noconfirm', '--distpath=dist/pyinstaller_dir']
        ...
        try:
            cmd = [sys.executable, '-m', 'PyInstaller', 'PokemonAnalyzer_dir.spec', '--clean', '--noconfirm', '--distpath=dist/pyinstaller_dir']
            if (UPX_DIR / "upx.exe").exists():
                cmd.append(f'--upx-dir={UPX_DIR.absolute()}')
...
def organize_distribution():
    ...
    exe_path = Path('dist/pyinstaller_dir/PokemonAnalyzer/PokemonAnalyzer.exe')
    ...
    print(f"[INFO] Taille EXE : {size_mb:.1f} MB (Dossier complet plus lourd)")

    # Crée un fichier README simple
    simple_readme = """# Pokémon Analyzer - Analyseur de Pokémon

## 🚀 Lancement

Double-cliquez sur **PokemonAnalyzer.exe** dans ce dossier.

## ⚠️ Notes

Ne déplacez PAS l'exécutable hors de ce dossier ! Il a besoin des autres fichiers présents ici pour fonctionner.
Si vous voulez un raccourci, faites Clic-droit > Envoyer vers > Bureau (créer un raccourci).
"""

    readme_path = Path('dist/pyinstaller_dir/PokemonAnalyzer/LISEZ-MOI.txt')
...
def show_summary():
    ...
    if exe_path.exists():
        ...
        print(f"[INFO] Format : ONE-DIR (Dossier complet)")
        print(f"[ASTUCE] Créez un raccourci vers cet .exe pour l'utiliser facilement.")
        return True
    return False
...
def main():
    """Fonction principale"""
    print("\n" + "="*70)
    print(" "*10 + "CRÉATION EXÉCUTABLE POKEMON ANALYZER (ONE-DIR)")
    print("="*70)
    ...
    # Résumé final
    print("\n" + "="*70)
    print(" "*15 + "COMPILATION TERMINÉE !")
    print("="*70)
    print(f"\n[INFO] Dossier final : dist/pyinstaller_dir/PokemonAnalyzer/")
    print("\n" + "="*70 + "\n")
